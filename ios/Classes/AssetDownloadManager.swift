...
let initCacheSize: Int = 500 * 1024 * 1024
...
public class AssetDownloadManager: NSObject {
    // MARK: - public
    ...
    public func setupConfiguration() {
        cache = URLCache(memoryCapacity: cacheSize, diskCapacity: cacheSize, diskPath: nil)
        let configuration = URLSessionConfiguration.background(withIdentifier: "t-osawa-009.AssetDownloadWrapper")
        
        configuration.urlCache = cache
        configuration.requestCachePolicy = .returnCacheDataElseLoad

        assetDownloadURLSession = AVAssetDownloadURLSession(configuration: configuration, assetDownloadDelegate: self, delegateQueue: OperationQueue.main)
        ...
        assetDownloadURLSession?.getAllTasks { tasksArray in
                ...
                for task in tasksArray {
                    guard let downloadTask: AVAssetDownloadTask = task as? AVAssetDownloadTask else { break }
                    // Restore asset, progress indicators, state, etc...
                    let asset: AVURLAsset = downloadTask.urlAsset
                }
            }
    }
    ...
    @objc public func setMaxCacheSize(_ maxCacheSize: NSNumber?){
        if let unsigned = maxCacheSize {
                    ...
                    cacheSize = Int(_maxCacheSize)
                    setupConfiguration()
                }
    }

    public static let shared = AssetDownloadManager()
   public static let options: [String : Any] = [AVAssetDownloadTaskMinimumRequiredMediaBitrateKey: 1_250_000]
    ...
    private var cache: URLCache = URLCache(memoryCapacity: initCacheSize, diskCapacity: initCacheSize, diskPath: nil)
    private var cacheSize: Int = initCacheSize
